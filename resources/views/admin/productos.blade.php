            ...
            <div class="bg-neutral-100 p-8 md:p-12 rounded-2xl shadow-xl w-full max-w-sm sm:max-w-md flex flex-col">
                <p class="text-xl font-bold text-gray-900 text-center mb-4">
                    <button onclick="eliminarTodo()"
                        class="bg-red-800 text-neutral-200 px-2 py-2 rounded hover:bg-red-900 transition"><svg width="18px"
                        ...
                        </svg></button> &nbsp; &nbsp; Productos
                </p>
                @if($productos->isEmpty())
                    <div class="flex flex-col items-center justify-center py-8">
                        <p class="text-gray-600 mb-4">No hay productos actualmente.</p>
                        <button class="bg-red-800 text-neutral-200 px-4 py-2 rounded hover:bg-red-900 transition"
                            onclick="document.getElementById('modal-producto').classList.remove('hidden')">
                            Añadir producto
                        </button>
                    </div>
                @else
                    ...
                    <button class="mt-6 bg-red-800 text-neutral-200 px-4 py-2 rounded hover:bg-red-900 transition"
                        onclick="document.getElementById('modal-producto').classList.remove('hidden')">
                        Añadir producto
                    </button>
                @endif
                ...
                <div id="modal-producto"
                    ...
                    <div
                        ...
                        <button class="absolute top-2 right-2 text-gray-400 hover:text-red-700 text-2xl"
                            onclick="document.getElementById('modal-producto').classList.add('hidden')">&times;</button>

                        <form method="POST" action="{{ route('productos.store') }}" class="flex flex-col gap-4">
                            @csrf
                            <h2 class="text-lg font-semibold mb-2 text-center text-red-800">Añadir producto</h2>

                            <div>
                                <label class="block text-sm font-medium text-gray-700">Nombre</label>
                                <input type="text" name="name" required
                                    class="mt-1 block w-full rounded border-gray-300 focus:border-red-600 focus:ring-red-600" />
                            </div>
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    fetch('{{ route("productos.delete") }}', {
                        ...
                        .catch(err => {
                            console.error(err);
                            Swal.fire('Error', 'Error al eliminar los productos.', 'error');
                        });
                }
            });
        }
